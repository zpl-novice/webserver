...
void webserver::reactor()
{
	...
	if(n==-1)
	    ...
	    {
		//setp();
		//cout<<endl<<p->serv_port;

		cout<<123;
		threadpool_add(thp,webserver::callback1,this);   //向线程池添加任务

	      //  web_accept(lfd);                //添加接受连接任务，在任务中调用accept
	    }
	    else                               //添加传输数据任务，在任务中调用read
	    {
	        sockfd=ep[i].data.fd;
...
void* webserver::callback1(void *arg){
	cout<<123;
	//((webserver *)arg)->web_accept(((webserver *)arg)->lfd);
	return NULL;
	}
